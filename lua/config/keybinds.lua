...
-- source file
-- Reload entire Neovim (recommended)
vim.keymap.set("n", "<leader><leader>", ":qa | nvim<cr>", { desc = "Restart Neovim" })

-- OR sync Lazy plugins only
vim.keymap.set("n", "<leader><leader>", "<cmd>Lazy sync<cr>", { desc = "Lazy sync" })

-- OR reload specific modules (manual)
